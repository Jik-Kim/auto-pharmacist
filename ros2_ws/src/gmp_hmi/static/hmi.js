...
function collectionBlocksOrder(s){return ['required','reset_pending'].includes(s.collection?.phase);}
function renderCollection(s){
 const c=s.collection||{},phase=c.phase||'unobserved',required=phase==='required',pending=phase==='reset_pending',available=phase==='available';
 $('collectionMode').textContent=required?'회수 필요':pending?'초기화 확인 대기':available?'주문 가능':'신호 대기';
 $('collectionMode').className='badge '+(required?'warn':pending?'info':available?'good':'neutral');
 $('collectionState').className='collection-state '+(required?'required':pending?'pending':available?'available':'');
 $('collectionState').textContent=c.message||'C 적재 카운터 신호 수신 대기';
 $('collectionHint').textContent=required?'QA가 완성품 패스박스와 폐기함을 모두 비운 뒤 회수 확인을 요청하세요.':pending?'HMI는 카운터를 직접 바꾸지 않습니다. C의 COLLECTION_RESET 수신 후 주문이 다시 가능합니다.':available?'C가 적재 카운터 초기화를 확인했습니다.':'C의 적재 카운터 이벤트를 받으면 현재 회수 상태를 표시합니다.';
}
...
function gate(){
 ...
 $('orderForm').querySelector('button').disabled=!enabled||snapshot.diagnostics?.actions?.run_batch!==true||!can(['operator'])||snapshot.batch_control?.submitting===true||!canStartOrder(snapshot.state)||!$('recipe').value||!selectedRecipe||collectionBlocksOrder(snapshot)||((source.demo||snapshot.inventory?.enforced)&&shortage().length>0)||(snapshot.inventory?.enforced&&!snapshot.inventory.fresh)||snapshot.inventory?.order_allowed===false||heightBlocked().length>0;
 ...
 const collectionRequestable=snapshot.collection?.phase==='required';
 $('openCollectionConfirm').disabled=!can(['qa'])||inFlight||!collectionRequestable;
 if($('collectionConfirm').open)$('confirmCollection').disabled=!can(['qa'])||inFlight||!collectionRequestable||!$('confirmPassboxEmpty').checked||!$('confirmRejectBinEmpty').checked;
...
function canStartOrder(st){return st?.mode==='IDLE'||(st?.mode==='DONE'&&['DONE','DISCARDED'].includes(st?.step));}
...
$('sessionButton').before(recoveryButton);
...
function renderSafetyPopup(s){
 ...
 $('safetyModeText').textContent=r.active||r.phase==='recovered'?`${r.reason} · ${r.message} · 상단 안전 복구에서 상세 확인`:triggered?'장치를 확인하세요. 필요 시 물리적 비상정지 버튼을 누르세요.':`${st.note||'일시 정지 상태'} · ${st.station||'위치 미확인'}`;
...
function render(s){if(!source.demo){const test=s.diagnostics?.namespace==='/hmi_test',stub=s.diagnostics?.transport==='inprocess';$('environment').textContent=stub?'가짜 ROS 대역 시험':test?'ROS 통신 시험':'실시간 연결';$('footerMode').textContent=stub?'HTTP · SQLite 실제 검증 / ROS 대역 사용 · DDS 미실행':test?'ROS 실제 통신 · 계량값은 시험 생성값 · 로봇 미연결':'공정 관측 · 요청 · 기록 조회';}snapshot=s;const st=s.state||{},f=s.freshness||{};fresh=f.state_age_s!=null&&f.state_age_s<=(f.stale_after_s??3);const completion=completionState(s),mode=fresh?completion.mode:'OFFLINE';$('mode').textContent=fresh?(modes[mode]||mode):st.mode?'상태 수신 중단':'상태 수신 대기';$('mode').className='state-pill '+mode;$('batch').textContent=st.batch_id||'—';$('item').textContent=st.batch_id&&st.item_index!=null?(Math.min(st.item_index+1,s.active_recipe?.items?.length||st.item_index+1)+(s.active_recipe?.items?.length?' / '+s.active_recipe.items.length:'')):'—';$('step').textContent=completion.pending?'물리적 완료 확인 대기':completion.terminal&&completion.discarded?'폐기 완료':steps[st.step]||st.step||'—';$('step').title=st.step||'';$('station').textContent=st.station||'—';$('note').textContent=fresh?(completion.pending?'용기 배출·폐기 등 물리적 작업 완료 신호를 기다립니다. 새 주문이 차단되어 있습니다.':((st.mode==='PAUSED'?(st.pause_reason==='NUDGE'?'접촉 감지 정지 · ':['REFILL','HEIGHT_LOW'].includes(st.pause_reason)?'원료 보충 대기 · ':''):'')+(st.note||'공정 상태 수신 중'))):'마지막 값입니다. 공정 상태가 다시 수신될 때까지 요청을 보낼 수 없습니다.';$('connection').textContent=fresh?'상태 수신 '+clock(st.t):'상태 미수신 · '+clock(st.t);
 ...
 const pending=(s.deviations||[]).filter(d=>d.decision==='PENDING'&&d.requires_decision);$('pendingCount').textContent='대기 '+pending.length+'건';$('deviations').innerHTML=pending.length?pending.map(d=>{const r=[...(s.results||[])].reverse().find(r=>r.batch_id===d.batch_id&&r.material_id===d.material_id);return `<article class="deviation"><h3>! ${escapeHtml(d.kind)}</h3><p>일탈 ${escapeHtml(d.deviation_id)} · 배치 ${escapeHtml(d.batch_id)}</p><p>${escapeHtml(d.detail)}</p>${r?`<p>원료 ${escapeHtml(d.material_id)} · 목표 ${number(r.target_g)} g / 실측 ${number(r.actual_g)} g</p>`:''}<div class="button-pair"><button data-dev="${escapeHtml(d.deviation_id)}" data-batch="${escapeHtml(d.batch_id)}" data-decision="1">승인 · 계속</button><button class="danger" data-dev="${escapeHtml(d.deviation_id)}" data-batch="${escapeHtml(d.batch_id)}" data-decision="2">폐기</button></div></article>`;}).join(''):'<div class="empty">✓ 판정 대기 일탈이 없습니다.</div>';renderInventory(s);renderCollection(s);renderProgress(s);renderDiagnostics(s);renderCompletion(s);renderSafetyPopup(s);renderCycle(s);renderNetwork();renderBatchControls(s);$('alarmCount').textContent=pending.length+heightBlocked().length+(fresh?0:1)+(st.mode==='ERROR'?1:0);gate();}
...
$('sessionButton').onclick=async()=>{if(source.demo){openUtilityDialog('administration');return;}if(!session.authenticated){$('loginOverlay').hidden=false;return;}try{await source.logout();session={authenticated:false,user:null};snapshot={};fresh=false;showSession();await source.session();}catch(e){handleAuthError(e);}};
