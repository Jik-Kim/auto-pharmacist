...
function showSession(){if(!session.authenticated){closeUtilityDialogs();for(const id of ['stockAlert','refillConfirm'])if($(id).open)$(id).close();}const user=session.user;$('loginOverlay').hidden=session.authenticated||source.demo;$('sessionButton').textContent=session.authenticated?`${user.username}${source.demo?' · 데모':' · 로그아웃'}`:'로그인';$('adminSession').textContent=source.demo?'데모 권한 미리보기 · 실제 인증/계정 생성 아님':user?user.username+' · '+user.role:'로그인 필요';gate();}
