...
| 운영 레시피 | PR #217로 `gmp_bringup/params/recipes`에 `recipe-01`~`03` 등록. 목표량·허용오차는 B 도징 보정 확정 전 잠정값 | 팀: 보정 확정 후 목표량/허용오차 재확인 |

HMI의 `integration`과 미연결 문구는 이 상태를 명시한다. 없는 ROS 엔드포인트를 만들거나
시험 서비스를 운영 네임스페이스에 붙이지 않는다.
잔량 카드·개별 보충·알림 열기 버튼은 화면에 **표시된다**. PR #176에서 숨겼다가 `bafb9b4`로 되살렸다 — 회수 카드를 되돌릴 때 같이 딸려 숨겨진 것이라 근거가 없었다. 시험 백엔드·API와 부족 차단/자동 알림도 그대로다.
회수 카운터·HMI 회수 확인 리셋은 SOT D-23 / #149 결정으로 구현하지 않는다. 회수 카드는 제거했으나 감사 기록만 남기던 `POST /collection-confirm` 도 9/23 제거했다(화면·공정 어디서도 쓰지 않았다). NUDGE_WAIT에서 회수 후 NUDGE로 진행한다.
할 일·이슈의 정본은 GitHub Issues다. `docs/todo.md`와 `docs/issues.md`는 동결된 과거 스냅샷이다.

## 검증

- HMI pytest 통과(ROS 스텁 포함, 작성 시점 109개 · 2026-09-23 기준 119개): 취소 권한/CSRF/actor, 늦은 수락, 취소 중복/UUID,
