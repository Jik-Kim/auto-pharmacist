...
PR #176에서 잔량 카드·개별 보충·알림 열기 버튼은 운영/시험/데모 모두 숨겼다. 시험 백엔드·API와 부족 차단/자동 알림은 유지하지만 화면에서 개별 보충을 완료할 수는 없다.
회수 카운터·HMI 회수 확인 리셋은 SOT D-23 / #149 결정으로 구현하지 않는다. 회수 UI·처리를 제거했으며, NUDGE_WAIT에서 회수 후 NUDGE로 진행한다.
