...
> 2026-09-23: 아래 원료 카드·개별 보충 버튼 절차는 현재 화면 그대로 쓸 수 있다. PR #176에서 한 번 숨겼다가 `bafb9b4`로 되살렸다. 시험 백엔드·API, 부족 차단과 자동 알림도 그대로다.
...
대기 또는 최종 DONE에서는 개별 보충 완료를 누른다. 진행 중이면 부족으로 멈췄더라도 ENTER 요청 후 진입 허가·PAUSED를 확인해야 한다. 공정이 스스로 원료 소진(`MATERIAL_EMPTY`)으로 보충 대기에 들어간 경우는 이미 안전 위치라 ENTER 없이 보충할 수 있다(ENTER 를 눌러도 「이미 대기 중」으로 수락). 이것은 QA 판정이 아니고 자동 복구(`AUTO_RECOVERED`)로 기록된다 — 실제 C 와 같다(`deviation.RULES`). 해당 원료를 만충 보충했음을 확인 대화상자에서 명시적으로 확인한다. 시험 화면에서는 가상 장부만 바뀐다.
