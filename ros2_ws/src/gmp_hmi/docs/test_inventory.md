...
`gmp_hmi` 내부 `/hmi_test` 전용 기능이다. 공용 `gmp_interfaces`와 실제 C의 `/cell` 계약은 변경하지 않는다. 시험 노드는 로봇·힘 센서·실제 도징을 호출하지 않는다. 실제 `/cell`은 재고·높이·보충 계약이 없으므로 **V4에서 새 주문을 차단**한다. 상태 조회·QA·인터락·기록은 유지한다.
