...
> 2026-09-21 PR #176: 이 문서의 원료 카드·개별 보충 버튼을 이용한 수동 시험 절차는 과거 UI 기준이다. 현재 잔량·보충 카드는 운영/시험/데모 모두 비표시이며 화면에서 개별 보충을 완료할 수 없다. 시험 백엔드·API, 부족 차단과 자동 알림은 유지한다. 아래 흐름은 시험 API 동작 설명으로만 참조한다.
