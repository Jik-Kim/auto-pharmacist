...
| 회수 확인 | SOT D-23 / #149에 따라 적재 카운터·HMI 회수 확인 리셋은 구현하지 않음. 회수 카드와 감사 기록만 남기던 `POST /collection-confirm` 모두 제거(엔드포인트는 9/23, 호출부 없음). NUDGE_WAIT에서 사람이 회수한 뒤 NUDGE로 진행 |
