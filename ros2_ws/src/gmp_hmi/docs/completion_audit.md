...
| 잔량·보충·높이 | 시험 재고 백엔드·데모 계산과 잔량 카드·개별 보충·알림 열기 버튼 모두 화면에 있음(PR #176에서 숨겼다가 `bafb9b4`로 복구). 운영 계약/연동은 미완료라 값은 시험·데모 데이터뿐 |
...
| 회수 확인 | SOT D-23 / #149에 따라 적재 카운터·HMI 회수 확인 리셋은 구현하지 않음. 회수 카드는 제거했으나 감사 기록만 남기는 `POST /collection-confirm`이 남아 있고 화면에서 호출하지 않음. NUDGE_WAIT에서 사람이 회수한 뒤 NUDGE로 진행 |
