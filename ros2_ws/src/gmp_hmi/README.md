...
**완성된 재고·높이·보충 흐름은 `/hmi_test`와 브라우저 데모용이다. 실제 `/cell`은 해당 C 계약이 없어서 V4에서 새 주문을 차단한다.** 기존 상태·QA·인터락·기록은 유지한다. 실제 C/로봇 코드·공용 메시지·좌표·도징 설정은 변경하지 않는다.

레시피: `recipe-01` A40/B40/C40, `recipe-02` A80/B40, `recipe-03` A40/B40/C80. 공통 C 로더와 기존 5% 오차를 사용한다. 실물 정밀도는 미검증. 기존 demo_batch는 `config/test_recipes`에 보관하며 선택 목록에는 새 3종만 표시한다.
