...
회수 카운터·HMI 회수 확인 리셋은 SOT D-23 / #149에 따라 구현하지 않는다. 화면의 회수 카드는 제거했으나 감사 기록만 남기는 `POST /collection-confirm`은 남아 있고 화면에서 호출하지 않는다. 세트 종료 NUDGE_WAIT에서 사람이 회수한 뒤 NUDGE로 진행한다. 재기동 화면은 저장 기록 조회만 제공한다.
