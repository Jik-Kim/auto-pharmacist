...
잔량 카드·개별 보충 버튼·부족 알림 열기 버튼은 **표시된다**. PR #176에서 한 번 숨겼으나 `bafb9b4`로 되살렸다 — 회수 카드를 되돌릴 때 같이 딸려 숨겨진 것이고 사람이 비표시를 결정한 근거는 없었다. 시험 백엔드·API와 부족 차단/자동 알림도 그대로다. 운영 재고값은 아직 ROS 계약이 없어 시험·데모 데이터로만 채워진다.
회수 카운터·HMI 회수 확인 리셋은 SOT D-23 / #149에 따라 구현하지 않는다. 화면의 회수 카드는 제거했으나 감사 기록만 남기던 `POST /collection-confirm` 도 9/23 제거했다(화면·공정 어디서도 쓰지 않았다). 세트 종료 NUDGE_WAIT에서 사람이 회수한 뒤 NUDGE로 진행한다. 재기동 화면은 저장 기록 조회만 제공한다.
