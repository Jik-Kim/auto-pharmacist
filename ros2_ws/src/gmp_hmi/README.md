...
## PR #176 화면 변경 (2026-09-21)

잔량 카드·개별 보충 버튼·부족 알림 열기 버튼은 운영/시험/데모 모두 비표시다. 시험 백엔드·API와 부족 차단/자동 알림은 유지하지만, 아래의 원료 카드 보충 절차는 현재 화면에서 실행할 수 없다. `tools/verify_ros_http.py`의 API 기반 시험과 구분한다.
회수 카운터·HMI 회수 확인 리셋은 SOT D-23 / #149에 따라 구현하지 않으며 회수 UI·처리를 제거했다. 세트 종료 NUDGE_WAIT에서 사람이 회수한 뒤 NUDGE로 진행한다. 재기동 화면은 저장 기록 조회만 제공한다.
