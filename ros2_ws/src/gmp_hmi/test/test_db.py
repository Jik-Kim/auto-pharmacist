...
def test_weight_subject_로_스쿱과_용기를_가른다(tmp_path):
    ...
    스쿱도 용기도 같은 scale 자세에서 재므로 station 으로는 갈리지 않는다 (계약 v1.2)."""
    db = _db(tmp_path)
    db.start_batch('B2', 200.0, 'DEMO')
    db.weight('B2', 201.0, 'scale', 70.0, 20.0, 50.0, 0.3, True, 'scoop')       # 붓기 전 스쿱
    db.weight('B2', 202.0, 'scale', 22.0, 20.0, 2.0, 0.3, True, 'scoop')        # 붓기 후 잔량
    db.weight('B2', 203.0, 'scale', 480.0, 30.0, 450.0, 0.5, True, 'container')  # VERIFY
...
def test_subject_를_안_주면_비어_있다(tmp_path):
    ...
    db.weight('B3', 301.0, 'scale', 80.0, 50.0, 30.0, 0.4, True)
