...
`resolvable()` 은 런타임에서 호출되지 않는다(호출처는 테스트뿐). `min_resolvable_g` 가 실제로 쓰이는 곳은
`process_fsm` 의 **VERIFY ②** 하나다 — 용기 순량과 스쿱 누적 투입량의 **차이** 임계다. 계량 한 번의
...
제안: 임계를 계량 횟수에서 유도한다 — `tol = k · σ · √(2n+2)` (σ = `min_resolvable_g`/3, n = 총 사이클).
`k = 3` 이면 오검출 0.27 % 로 고정되고 탐지력은 배치마다 최선이 된다. 근거·수치는
`config/scale_reference.yaml` 의 `verify_mismatch` 절. **수식은 C, 파라미터 추가는 조장 소관이라 B 는 근거만 낸다.**
