...
```
 ...
 PICK_SCOOP ──grip ok──▶ SCOOP_TARE ──weigh_scoop──▶ SCOOP ──contact──▶ WEIGH_SCOOP ──weigh_scoop──▶ POUR ──▶ WEIGH_RESIDUAL
   ...
   계량 무효(valid=false): 각 계량 상태에서 같은 요청 재시도 ≤2, 3회째 WEIGH_INVALID → 투입 전은 CLEANUP → ERROR · 투입 후는 DEVIATION   └─ 9회째 UNDER ▶ DEVIATION(TIMEOUT)
   빈 스쿱(SCOOP_EMPTY)의 증거는 둘이다 — SCOOP 의 contact=false(깊이 보정 모드만) 또는 WEIGH_SCOOP 순중량 ≤ empty_scoop_g(모드 무관).
   어느 쪽이든 같은 scoop 재시도 ×3, 4회째 MATERIAL_EMPTY → PAUSED(보충). 고정 모드(dosing.fixed_scoop)는 SCOOP 에서 접촉을 안 보고 곧장
   WEIGH_SCOOP 으로 가며, 깊이는 첫·반환 뒤·보충 모두 1.0 이다 (D-34, #282·#289)
