...
- [ ] **[Q-12]** Pass Box 의 「완성품」·「빈통」이 `output_tray`·`magazine` 을 대체하는지 확정 — 대체면 FSM 의 `carry` 목적지 두 곳이 바뀐다 · 마감 9/21 (조장과)
- [ ] **[D-24]** `nudge_wait` 스테이션 추가 — 세트 완료 후 이동해 NUDGE 대기. `safe` 와 별개 자리 (판 우상단) · 마감 9/21
