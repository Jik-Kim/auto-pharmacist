...
- [ ] **[Q-12]** D-24 스테이션 매핑 확정 — (a) Pass Box 「완성품」·「빈통」이 `output_tray`·`magazine` 을 대체하는지 (대체면 FSM 의 `carry` 목적지가 바뀐다) (b) 「넛지 대기 위치」 스테이션 ID·진입 시점 (c) `material_4`(예비) 제거 여부 · 마감 9/21 (조장과)
