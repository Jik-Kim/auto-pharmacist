...
- [x] **`WeighHeld`**: 들고 있는 스쿱을 계량 자세로 이동해 `samples` 회 읽고, 파지·내려놓기·원위치 복귀 없이 계량 자세에 머물도록 구현. 빈 그리퍼 실패, `LIFT/SETTLE/MEASURE`, `subject=scoop` 설정과 단위 테스트 완료(9/19). **스쿱 파지 후 첫 호출은 BASE +Y 150 mm 인출을 먼저 수행**한다. 실물 궤적·간섭 검증은 보류 · 마감 9/21
