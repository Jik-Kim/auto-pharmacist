...
2. **오늘 인계 — 조장·A·C·D:** 계약 v1.2·v1.3 팀 승인 완료(9/20 사용자 확인). 원료 반환 6개 자세 티칭 준비, HMI 반환 상태·결과 표시 인계. 코드 구현과 계약 확정은 별개다.
...
**전체 57/102 완료** (███████████░░░░░░░░░)  ·  기준 09/20

| 파트 | 완료 | 진행 | 지난 마감 |
|---|---|---|---|
| gmp_interfaces [조장] | 8/10 | `████████░░` | **1** |
...
## gmp_interfaces [조장]
- [ ] **[v1.4 안전 복구]** `RecoverSafety.srv`·이벤트 코드·상태별 복구 의미를 A 구현과 함께 문서화. HMI 요청 운영 팀 합의 확인, 세부 필드 및 C/D 연동 검토 남음 · 마감 9/21
...
## gmp_process [C 공정]
- [ ] **[v1.4 안전 복구 인계]** 안전 이벤트를 ERROR·주문 차단으로 연결, FORCE_LIMIT 재시도 제외. HMI→C→A 복구 중계·권한/배치 확인·명시적 새 시작 정책 구현 (`docs/interfaces.md` 8절) · 마감 9/21
