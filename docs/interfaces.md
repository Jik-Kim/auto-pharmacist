...
- HMI 조작은 `CellEvent(code='HMI_ORDER'|'HMI_QA_APPROVE'|'HMI_QA_DISCARD'|'HMI_INTERLOCK_ENTER'|'HMI_INTERLOCK_EXIT'|'HMI_COLLECTION_CONFIRMED', text='<actor> <detail>')` 로 발행한다. actor 가 비면 `unknown` — 시연에서는 반드시 ID 를 넣는다.
- **적재·회수 (Pass Box 회수 운영)**: 완성품 칸·폐기함은 구멍 하나에 통 하나라 용량이 각각 1 이다(`capacity.passbox_done`·`capacity.reject_bin`, `common.yaml`. `stations.yaml` 의 `slots` 는 물리 칸 수로 **다른 값**이다. 0 이하면 그 칸은 한도 검사를 끈다). process 는 용기를 실제로 놓았을 때만 세고 `CellEvent(code='LOADED', text='<station> <n>/<cap>')`, 한도에 닿으면 `CellEvent(code='COLLECTION_REQUIRED', level=WARN, text='<station> 가득참 (<n>/<cap>) — QA 회수 확인 필요')` 를 낸다. 진행 중 배치는 끝까지 마치고, **다음 주문을 접수에서 거부**한다(다 만들고 나서 갈 데가 없는 것보다 낫다). 리셋은 HMI 가 내는 `HMI_COLLECTION_CONFIRMED` 를 process 가 구독해 처리한다 — 별도 서비스가 없다. HMI 는 두 칸을 **모두** 비웠을 때만 그 이벤트를 내므로(`/collection-confirm` 이 두 플래그를 모두 요구) process 도 둘 다 0 으로 돌리고 `CellEvent(code='COLLECTION_RESET', text='회수 확인(<actor>) — 적재 카운터 초기화 …')` 를 낸다. 이미 0 이어도 무해하다(중복 확인 허용). 카운터는 노드 메모리에만 있어 **재기동하면 0 부터다** — 재기동 이어하기(추가 3)에서 같이 다룬다.
