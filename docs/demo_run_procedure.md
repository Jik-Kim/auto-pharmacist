...
| T1 | 브링업 | **시연용 기록 DB 새로 시작** — 이전 런치가 모두 꺼진 상태에서 `B=~/auto-pharmacist/records/backup_$(date +%m%d_%H%M); mkdir -p $B && mv ~/auto-pharmacist/records/cell.db* $B/ 2>/dev/null` (WAL `cell.db-wal`·`-shm` 까지 함께 옮긴다. 안 옮기면 리허설·시험 배치가 KPI 카드·배치 이력에 섞인다. 새 DB 는 `record_node` 가 기동 시 만든다) → `ros2 launch gmp_bringup cell.launch.py mode:=real host:=192.168.1.100` | `[skill_node] SELF_CHECK OK` 로그 (툴·TCP·충돌 감도 일치) |
