...
## 높이 기반 스쿠핑 보정·인계 (9/22 기록)

> 9/23 고정 경로 운용은 아래 「DRL 고정 경로·DIO 운용」을 우선한다. 이 절은 보류된 높이 보정 모델의 기록이다.
...
2. **C 자동 공정 연결 (D-34, #282).** `dosing.fixed_scoop=true`(common.yaml)면
   깊이는 첫·반환 뒤·보충 모두 1.0 이고(#289), FSM 은 contact_detected 를 진행 조건으로 쓰지 않는다.
   고정 경로의 미측정과 접촉 실패는 **이어지는 WeighHeld 순중량**으로 가른다 —
   `dosing.empty_scoop_g` 이하면 SCOOP_EMPTY(재시도 ×3, 4회째 MATERIAL_EMPTY).
   true 접촉값을 만들지 않고, `process_node.py`는 여전히 Scoop.message를 FSM에 전달하지 않는다.
   ⚠️ `empty_scoop_g` 는 잠정값이다 — 빈 스쿱 계량 산포는 아직 미측정.
