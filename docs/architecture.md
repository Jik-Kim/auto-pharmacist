...
| 12 | 다음 원료 → 5 | | |
| 13 | `VERIFY` | `MeasureForce` (들기 전 빈 그리퍼 영점 재확인) → `WeighContainer(tare_g)` — **용기를 들어** 계량 (그리퍼 비어 있음) | **들기 전 영점 재확인**: `MeasureForce` 결과를 SELF_CHECK 의 빈 그리퍼 값과 대조해 `|이동| > scale.zero_drift_limit_n`(기본 0.5 N, 0 이면 끔)이면 재측정, `max_invalid` 도달 시 `Deviation(WEIGH_INVALID)` → QA. NUDGE 는 정지·재개 장치일 뿐 계량 유효성과 연결돼 있지 않아 오염된 값이 그냥 장부에 들어가던 구멍을 막는다. 통과하면 계량한다. **① 제품 판정 하나만 한다** (9/22 사용자·조장 확정, 종전 ② 폐지). `\|net − Σtarget\| > Σ(target×tol)` → `Deviation(BATCH_OUT_OF_SPEC)` → QA (폐기 권고). ~~② 계측 신뢰성 `\|net − Σ투입량\|`~~ 은 **판정하지 않고 관측만** 한다 — 값은 `verify_detail` 에 담겨 `CellEvent(INFO, VERIFY)` 로 나간다. ② 를 끈다는 것은 **배치 기록 교차검증을 포기한다**는 뜻이다 (제품은 규격 안인데 원료별 투입 기록이 틀린 배치를 검출할 수단이 없어진다) |
