...
| 7 | `SCOOP` | `Scoop(material_id, depth_fraction)` — 깊이 비율은 v1.5, 실제 Z 변환은 A 실물 뒤 | 깊이 보정 모드: `contact_detected=false` → `SCOOP_EMPTY` → 재시도 ×3, 4회째 → `MATERIAL_EMPTY` → 인터락 보충 요청. **고정 모드(`dosing.fixed_scoop`)는 접촉을 안 보고** 8 의 순중량으로 가른다(#282), 깊이는 늘 1.0 |
| 8 | `WEIGH_SCOOP` | `weigh_scoop`(붓기 전) | 퍼낸 양 = gross − 스쿱 풍량. **퍼낸 양 ≤ `empty_scoop_g`** 면 빈 스쿱 → 7 과 같은 SCOOP_EMPTY 경로(모드 무관, #282). **퍼낸 양 > 남은 목표 + target×tol** 이면 8a 반환, 아니면 9 전량 붓기 — 초과 예방 (1차 폐루프, v1.3) |
| 8a | `RETURN_MATERIAL` | `ReturnMaterial(material_id)` — `return_start_posx` 직선 → `return_end_posj` 관절, 끝 자세 유지 (v1.5.1) | 성공 → 7 재스쿱 (깊이 = 직전 × 남은량/퍼낸 양, 하한 `min_fraction` · 고정 모드는 1.0), returns ≥ `max_returns`(기본 3 — 붓기 상한 `max_attempts` 와 **분리**돼 있다, #189) → `Deviation(TIMEOUT)`. 실패 → `safe` 후 `ERROR`. **연결 경로 구현 전까지 실물 skill_node 는 후속 Scoop 을 거부한다** |
