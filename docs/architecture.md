...
| 7 | `SCOOP` | `Scoop(material_id, depth_fraction)` — 깊이 비율은 v1.5, 실제 Z 변환은 A 실물 뒤 | `contact_detected=false` → `SCOOP_EMPTY` → 재시도 ×3, 4회째 → `MATERIAL_EMPTY` → 인터락 보충 요청. **고정 모드(`dosing.fixed_scoop`)는 접촉을 안 보고** 8 의 순중량으로 가른다(#282), 깊이는 늘 1.0 |
