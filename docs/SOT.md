...
| D-26 | **VERIFY ② (계측 신뢰성 대조) 폐지 — VERIFY 는 ① 제품 규격만 판정** (9/22 B·C 임시 담당 김병직 제안, 조장 승인) | 9/22 실측(B, `scale_reference.yaml`): 스쿱 계량 자세 material_1/2 는 손목 분기·공구 무게중심(cz 89) 을 바로잡아도 계량 요동 σ 19~25 g 이고 material_3 만 6 g 이다(기전 미규명, A 인계). ② 가 살려면 스쿱 계량 σ_s < 1.5 g 이 필요해 **값 조정으로는 살릴 수 없다**. ① 은 용기 계량(`WeighContainer`)만 쓰므로 스쿱 자세와 무관하다. **결정**: (1) VERIFY 는 ① `|net − Σtarget| > Σ(target×tol)` → `BATCH_OUT_OF_SPEC` 만 판정한다. (2) ② 수치 `net − Σ투입량` 은 판정 없이 `detail`·CellEvent 에 **기록만** 한다. (3) `min_resolvable_g` 는 ② 의 유일한 실사용이었으므로 제거한다(`skill_node` 가 선언 없이 읽으므로 그쪽 먼저). (4) 9/21 「계량은 material_3 한 자세에서만」(BRD 3.4.3) 제약은 **조장 결정으로 해제한다** — 스쿱 계량은 초과 예방·반환 판정용이라 자기 원료 자세에서 재는 것을 목표로 한다. **단, 현 실측으로는 material_1/2 를 계량 자리로 쓸 수 없다**(B 재분석, CSV 47개·회차 449건(자리별 통계는 표본 12개 이상 213건 기준), 9/22: material_1 운영 조건 samples 20 무효율 50 %(7/14) → `max_invalid` 2 기준 원료 1종 통과 확률 ≈ 31 %, 회차간 σ 4.3~5.7 g 은 4 가지 피팅으로도 안 내려가며 원료 허용폭 ±5 g 과 같은 크기라 `decide()` 대략치로도 부족. 고주파 게이트 `max_hf_std_g` 는 조작 검출이지 잡음 저감이 아님). 따라서 해제는 **조건부**다: material_1/2 자세에서 유효율·σ 를 쓸 만하게 만드는 방법(B 과제 — 자세·공구 보상·계량 창·필터, material_3 만 좋은 기전 규명은 A)이 나오기 전까지 운영은 material_3 자세를 그대로 쓴다. **포기하는 것을 명시한다**: ② 는 제품이 아니라 **배치 기록**을 지키는 검사였다. 끄면 「용기 순량은 규격 안인데 `ScoopCycle`·`dispense_result` 의 원료별 투입량은 틀린 배치」(흘림·스쿱 편향·붓기 손실이 상쇄된 경우)를 검출할 수단이 없다. 배치 기록의 교차검증 요구가 다시 올라오면 이 결정을 다시 연다. 계약 영향: `Deviation.kind VERIFY_MISMATCH` 열거값은 **유지**(미발생 표시만), v1.5.1 그대로. 근거: 9/22 검토 종합(`Cal1_proj/notes/07`), PR #203 `decision_0922_verify2_off` | 김병직 + 조장 | 9/22 |
