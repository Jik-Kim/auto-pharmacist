...
| Q-12 | **Pass Box 의 「완성품」·「빈통」 칸이 기존 `output_tray`·`magazine` 을 대체하는가** — 대체라면 FSM 의 `carry('magazine'→'scale')`(빈 통 가져오기)·`carry('scale'→'output_tray')`(완성품 내려놓기) 목적지가 Pass Box 두 칸이 된다. 별개라면 판 어딘가에 매거진·트레이가 따로 있어야 하는데 D-24 그림에는 없다 | C + 조장 | 9/21 |
