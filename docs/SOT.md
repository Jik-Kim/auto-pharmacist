...
- **C/D 인계 대기:** ReturnMaterial 성공 시 반환 끝 자세에 남고 RETURN 피드백은 발생하지 않는다. docs/interfaces.md의 기존 왕복 설명과 차이가 있으며 팀 공유 후 계약 설명을 정정해야 한다. 메시지 필드는 변경하지 않았다. 재스쿱 자동 연속 운용은 새 연결 경로 구현·검증 후 가능하다.
