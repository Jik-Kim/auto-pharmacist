...
| D-08 | 도징 단위 | **시연 레시피는 A/B/C = 200/150/100 g, 각 허용 오차 ±5 %**다. 30 g/100 g 중 하나를 고르는 초기안은 폐기한다. **G1 결과 (9/18, tool_force, 스쿱+원료 총 133 g (빈 스쿱 32 g), 6세트×30회×10표본)**: 계량 한 번(회차 평균)의 σ = 6.0 g, **3σ = 18.0 g** → `scale.min_resolvable_g = 19`. **9/19 재측정 (32 g·132 g 각 3세트×30회, 표본 간격 0.82 s 로 독립)**: 3σ = **12.6 / 12.4 g**, 두 점 직선 **gain 0.886 · offset 247.1** (100 g 차이가 센서 113 g). 0.82초 표본 간격은 코드·설정에 반영했지만 실물 재측정 전까지 분해능은 19 g을 유지하며, 재측정 뒤 14 g 적용 여부를 판단한다. 원료별 ±5 % 허용 폭은 한 번의 스쿱 계량 분해능보다 작으므로 스쿱 계량은 초과 예방·투입량 추정에 쓰고, 합격 판정은 배치 끝 VERIFY의 ① 레시피 총량 대조와 ② 누적 투입량 대조로 한다(Q-11). 근거 데이터·재계산: `gmp_dosing/calibration/`, `core/calib.py`, `config/scale_reference.yaml` (PR #22) |
...
- 현재 접촉 위치·삽입 깊이 확인 동작의 내부 이름은 `check_depth`(`_do_check_depth`)다. 기존 `_do_scoop`에서 이를 호출한다.
- 외부 `Scoop` Action·필드·피드백·작업명·오류 이벤트·설정값은 유지하므로 다른 담당 파트의 변경은 필요하지 않다. 이동 경로와 힘제어도 그대로다.
- 실제 원료를 퍼 올리는 스쿠핑과 `depth_fraction` 반영은 미구현이다. 내부 이름 분리는 이 기능들의 구현 또는 실물 검증 완료를 뜻하지 않는다.
