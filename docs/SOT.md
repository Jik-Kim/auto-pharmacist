...
- **현재 calibrated=false:** BASE=WORLD 가정 시 제공된 근사 오프셋으로 mid 끝 높이48.6 mm가 나와 실측54~55 mm와 불일치한다. 실제 좌표 변환·오프셋·기준 표면 보정 전에는 첫 이동부터 거부한다. true만 바꿔도 경유점이 바닥50+여유5 mm를 침범하면 거부한다. 이 여유는 실측4~5 mm 중 보수적 값이며 전체 spline 간섭 보장이 아니다.
