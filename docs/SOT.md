...
| Q-10 | **추가 기능 7 의 방식** — `wait_nudge()` 는 DRL 전용, 파이썬 API 에 없음. 동등 구현: `get_tool_force` 폴링으로 순간 외력(예: 8 N 이상 0.2 s) 감지. 반응 (a) 건드리면 일시정지·다시 건드리면 재개 (b) 부딪히면 안전 자세 후퇴 → 인터락 EXIT 로 재개 (c) 둘 다 + 속도 제한 | 팀 | 9/17 |
