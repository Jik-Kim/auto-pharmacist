...
| 일 | 담당 | 비고 |
|---|---|---|
| 스테이션 물리 배치 (판 **450×450**, 로봇 좌측 28 cm 이격, 중앙 300×300 Keep Clear — SOT D-24), Pass Box 2칸·폐기함·넛지 대기 위치 고정, 원료통·전용 스쿱은 판 바깥 아래 | C + 전원 | 9/18 확정 (820×650 판·매거진·트레이는 폐기). 판이 밀리면 전부 틀어진다 — 테이프로 위치를 표시 |
| 티칭 (`stations.yaml`) | A | 로봇 BASE 좌표(`frame: base`)로 `posx` 를 그대로 티칭한다 — D-15 판 좌표계는 쓰지 않는다 (9/18 확정, D-24). 9/21 좌표 갱신·반환 자세 입력 완료, 실물 간섭 확인 대기 |
