...
**9/23 기준.** 계약 **v1.8**(`DispenseResult.verdict` INVALID 추가, #241), 패키지 6개(interfaces·skills·dosing·process·hmi·bringup) 구현·통합 시험 완료(gmp_process 176/7 skipped 기준선), PR #241 까지 머지. 실물 상태: 공구 `tool_weight` 1.36 kg · CoG [5.31, −34.68, 8.28] 로 펜던트 동결(9/23, cz 89 철회 #235); 계량 `scale.gain 0.886·offset 247.1` 은 우드락 공구(9/19) 값이라 PLA 공구에서 **재보정 전 미검증**(#187); 로봇 재파지 σ 6.99 g. 레시피는 `gmp_bringup/params/recipes/recipe-01~03.yaml`(총 120/120/160 g, ±5 %) — 9/21 의 200/150/100 g 은 폐기값이며 확정은 #53. 이 값에서는 VERIFY ① 허용폭(6~8 g)이 재파지 σ 보다 작아 **① 이 성립하지 않는다**(SOT D-26 9/23 추기, 조장 판단 대기). 스쿠핑 높이 보정은 `calibrated=false` 로 보류(#236). 계량 무효 정책(#213)·원료 소진 `MATERIAL_EMPTY`(#111) 반영. 현황은 [GitHub Issues](https://github.com/Jik-Kim/auto-pharmacist/issues), 확정 결정은 `docs/SOT.md`, 파트별 현행값은 `practice/<파트>/CURRENT.md`.
