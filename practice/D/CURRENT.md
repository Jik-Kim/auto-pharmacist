...
| 배치 완료·폐기 알림 | 모든 화면에서 **떠 있는 알림**(PC 왼쪽 아래·휴대폰 아래), `COMPLETION_SHOW_MS` 8 s 뒤 자동 숨김, ✕ 닫기, 같은 배치·결과로는 다시 안 뜸. 칸 사이에 끼우지 않는다(아래 카드를 밀었다) | `hmi.js` `renderCompletion`, 이 PR | PR #229 (`hmi.css:1`·`:73`), 통합 전 정리 PR(`hmi.css` 끝 모바일 블록) |
