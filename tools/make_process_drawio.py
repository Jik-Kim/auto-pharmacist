...
selfc = S(1, 0, 'SELF_CHECK', 'req: measure')
pickc = S(2, 0, 'PICK_CONTAINER', 'req: carry magazine→scale')
tare = S(3, 0, 'TARE', 'req: weigh(tare 0)')
picks = S(1, 1, 'PICK_SCOOP', 'req: move(scoop_rack) → grip')
scoop = S(2, 1, 'SCOOP', 'req: scoop(material, attempt)')
pour = S(3, 1, 'POUR', 'req: pour(scale, fraction)')
weigh = S(4, 1, 'WEIGH', 'req: weigh(scale, tare) → decide()')
ret = S(2, 2, 'RETURN_SCOOP', 'req: move(scoop_rack) → grip(open)')
...
disc = S(5, 3, 'DISCARDED', 'req: carry scale→reject_bin', 'e')
...
p2.edge(tare, picks, 'tare_g 저장 · scale.set_tare() · cur = 원료 0 · weight 발행', color=ACC,
        exit=(0.5, 1), entry=(0.15, 0), points=((cx(3), 300), (cx(1, 0.15), 300)), lpos=(0, -14))
p2.edge(picks, scoop, 'grip_inferred=true · attempts=1', color=ACC, lpos=(0, 16))
p2.edge(scoop, pour, 'contact_detected=true · fraction 전달', color=ACC, lpos=(0, 16))
p2.edge(pour, weigh, '', color=ACC)
p2.edge(weigh, ret, 'OK (|err| ≤ tol) → dispense_result 발행', color=OK,
        exit=(0.35, 1), entry=(1, 0.5), points=((cx(4, 0.35), 600), (X[2] + W + 40, 600), (X[2] + W + 40, Y[2] + 32)), lpos=(-0.2, -14))
p2.edge(ret, picks, 'grip(open) · idx+1 · 다음 원료 있음', color=ACC,
        exit=(0, 0.5), entry=(0.5, 1), points=((cx(1), Y[2] + 32),), lpos=(0.3, -14))
p2.edge(ret, finish, '마지막 원료였음', color=ACC, lpos=(0, -14))
p2.edge(finish, done, 'carry ok', color=OK, lpos=(0, -14))
# ── 보정·재계량 (주황·회색) — 1행 위 복도 (y 340~370)
p2.edge(weigh, scoop, 'UNDER, attempts<3 → scoop(fraction = 부족량 / 스쿱 1회량)', color=WARM,
        exit=(0.3, 0), entry=(0.75, 0), points=((cx(4, 0.3), 340), (cx(2, 0.75), 340)), lpos=(0, -14))
p2.edge(weigh, weigh, 'INVALID (valid=false) ≤2 → 재계량', color=GRAY,
        exit=(0.75, 0), entry=(1, 0.3), points=((cx(4, 0.75), 365), (X[4] + W + 30, 365), (X[4] + W + 30, Y[1] + 19)), lpos=(0.3, 14))
p2.edge(weigh, dev, 'OVER → OVERFILL\nUNDER 4회째 → TIMEOUT\nINVALID 3회 → WEIGH_INVALID', color=WARM,
        exit=(1, 0.6), entry=(1, 0.5), points=((X[4] + W + 60, Y[1] + 38), (X[4] + W + 60, Y[3] + 32)), lpos=(-0.55, -70))
...
# ── QA 판정 (DEVIATION)
p2.edge(dev, ret, 'APPROVED → 결과에 남기고 스쿱 반납', color=OK,
        exit=(0, 0.5), entry=(0.5, 1), points=((cx(2), Y[3] + 32),), lpos=(-0.3, -14))
p2.edge(dev, disc, 'DISCARDED\n용기째 폐기함', color=RED, lpos=(0, -22))
# ── 강제 개입 (ERROR) — 왼쪽 복도 x=200
p2.edge(pickc, err, 'GRIP_FAIL 4회 (FORCED)', color=RED,
        exit=(0.15, 1), entry=(0.5, 0), points=((cx(2, 0.15), 230), (200, 230), (200, Y[3] - 30), (cx(0), Y[3] - 30)), lpos=(-0.6, -14))
p2.edge(picks, err, 'GRIP_FAIL 4회 (FORCED)', color=RED,
        exit=(0, 0.5), entry=(1, 0.3), points=((240, Y[1] + 32), (240, Y[3] + 19)), lpos=(0.2, -50))
p2.edge(selfc, err, 'measure invalid (TODO)', color=RED,
        exit=(0, 0.7), entry=(0.3, 0), points=((cx(0, 0.3), Y[0] + 45),), lpos=(0.5, -14))
