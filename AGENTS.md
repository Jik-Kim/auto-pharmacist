...
- **설정 dataclass(`ScaleConfig`·`DosingConfig`·`ProcessFSM` 인자 등)는 키워드로만 넘긴다.** 기본값이 있는 dataclass 는 필드를 하나 빼거나 끼워 넣어도 인자 수가 맞아 오류가 안 나고, 밀린 값이 그럴듯해서 시험도 통과한다 — 9/22 에 `min_resolvable_g` 를 빼면서 `max_std_g`(계량 `valid` 의 유일한 손잡이)가 한 칸 밀릴 뻔했고(#209), `DosingConfig` 도 같은 패턴이었다(#210). 위치 인자로 넘기는 호출부를 보면 그 PR 에서 키워드로 바꾼다.
...
## 작업 일지 — `practice/` (9/23 조장 승인)

- 세션(사람·Claude)이 자기 작업을 잊고 재작업하는 것을 막는다. `practice/<파트>/CURRENT.md`(현행값 한 장) 와 `practice/<파트>/YYYY-MM-DD_<주제>.md`(시행착오 일지). 역할 분담: SOT = 결정, CURRENT = 현행값, 일지 = 과정. 규칙은 `practice/README.md`.
- **시작**: 자기 파트 `CURRENT.md` 를 읽는다. 값을 주장하기 전에 저장소 원본(yaml·레시피·계약 문서)을 직접 연다 — 9/23 에 레시피가 바뀐 것을 모른 채 옛 허용폭으로 이틀을 계산했다.
- **종료**: 일지 추가 + `CURRENT.md` 갱신 + 커밋·푸시. 틀린 결론은 지우지 않고 「철회(날짜, 근거)」로 남긴다. 값에는 근거 링크를 붙인다.
- 일지는 docs 전용이라 코드 PR 검토 파이프라인 면제, 하루치를 PR 하나로. **`CURRENT.md` 는 코드 PR 이 현행값을 바꾸면 같은 PR 에서 함께 고친다**(9/23 팀장 결정 — 따로 미루면 놓친다, `practice/README.md` 규칙 6).
